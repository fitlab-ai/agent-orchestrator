...
template_version=$(
  ...
  echo "Error: Failed to read templateVersion from .airc.json."
  exit 1
}
...
if ! printf '%s\n' "$template_version" | grep -Eq '^v[0-9]+\.[0-9]+\.[0-9]+$'; then
  echo "Error: .airc.json templateVersion must use v-prefixed semver (found: $template_version)."
  echo "Fix: set .airc.json.templateVersion to v$package_version"
  exit 1
fi
...
if [ "${template_version#v}" != "$package_version" ]; then
  echo "Error: .airc.json templateVersion and package.json version do not match."
