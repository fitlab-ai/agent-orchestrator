...
## Step 7: 必须且只能选择一个输出分支

按以下顺序判断：
1. 如果 `Blocker = 0` 且 `Major = 0` 且 `Minor = 0`，使用 Branch A
2. 如果 `Blocker = 0` 且（`Major > 0` 或 `Minor > 0`），使用 Branch B
3. 如果 `Blocker > 0`，且问题可以通过一次聚焦修复解决，使用 Branch C
4. 如果任务需要重大重构、大范围重写或整体重来，使用 Branch D

禁止规则：
- 不要跳过分支判断步骤
- 不要混用不同分支的文案
- 只要 `Blocker > 0`，就绝对不能输出通过模板
- 所选分支中必须包含所有 TUI 命令格式
