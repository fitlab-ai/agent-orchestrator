...
export function syncGpgKeys(container, home, project, execFn = execFileSync, runSafeFn = runSafe) {
  const hostEnv = { ...process.env, HOME: home };
  const cached = readGpgCache(home, project, execFn);
  const pubKeys = cached?.pub ?? execFn('gpg', ['--export'], {
    env: hostEnv,
    stdio: ['ignore', 'pipe', 'pipe']
  });
  ...
  const secKeys = cached?.sec ?? execFn('gpg', ['--export-secret-keys'], {
    env: hostEnv,
    stdio: ['ignore', 'pipe', 'pipe']
  });
  ...
  if (!cached) {
    ...
    if (fingerprint) {
      writeGpgCache(home, project, pubKeys, secKeys, fingerprint);
    }
  }
...
export function ensureClaudeOnboarding(toolDir) {
  ...
  if (!workspace.hasTrustDialogAccepted) {
...
export function extractClaudeOAuthToken(home, execFn = execFileSync) {
  ...
  if (process.platform === 'darwin') {
    try {
      ...
      const parsed = JSON.parse(credentials.trim());
      ...
      },
      {
        ...
        task: async (message) => {
          ...
          if (claudeCodeEntry) {
            ...
            const claudeToken = extractClaudeOAuthToken(effectiveConfig.home);
            if (claudeToken) {
              envArgs.push('-e', `CLAUDE_CODE_OAUTH_TOKEN=${claudeToken}`);
            }
          }
          ...
          if (needsGpg) {
            const hasCachedGpgKeys = readGpgCache(effectiveConfig.home, effectiveConfig.project) !== null;
            message(
              hasCachedGpgKeys
            ...
            try {
              if (syncGpgKeys(container, effectiveConfig.home, effectiveConfig.project)) {
