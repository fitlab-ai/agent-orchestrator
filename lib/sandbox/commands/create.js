...
export function buildContainerEnvArgs(resolvedTools, runSafeCommand = runSafe, engine = null, runSafeEngineFn = runSafeEngine) {
  const envArgs = resolvedTools.flatMap(({ tool }) =>
    Object.entries(tool.envVars ?? {}).flatMap(([key, value]) => ['-e', `${key}=${value}`])
  );
  const ghToken = engine
    ? runSafeEngineFn(engine, 'gh', ['auth', 'token'])
    : runSafeCommand('gh', ['auth', 'token']);
  if (ghToken) {
      ...
      },
      {
        ...
        task: async (message) => {
          ...
          const cachedGpg = needsGpg
            ...
            : null;
          const envArgs = buildContainerEnvArgs(resolvedTools, runSafe, engine);
