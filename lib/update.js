...
const CONFIG_DIR = '.agent-infra';
const CONFIG_PATH = path.join(CONFIG_DIR, 'config.json');
const LEGACY_CONFIG_PATH = '.airc.json';
const WORKSPACE_PATH = path.join(CONFIG_DIR, 'workspace');
const LEGACY_WORKSPACE_PATH = '.agent-workspace';
...
function migrateLegacyPaths() {
  ...
  if (!fs.existsSync(WORKSPACE_PATH) && fs.existsSync(LEGACY_WORKSPACE_PATH)) {
    fs.mkdirSync(CONFIG_DIR, { recursive: true });
    fs.renameSync(LEGACY_WORKSPACE_PATH, WORKSPACE_PATH);
    migratedWorkspace = true;
  }

  return { migratedConfig, migratedWorkspace };
}
...
function syncFileRegistry(config) {
  ...
  const normalizeList = (list = []) => list.filter((entry) => !RETIRED_FILE_ENTRIES.has(entry));

  delete config.modules;
  config.files.managed = normalizeList(config.files.managed);
  config.files.merged = normalizeList(config.files.merged);
  config.files.ejected = normalizeList(config.files.ejected);
...
async function cmdUpdate() {
  ...
  const { migratedConfig, migratedWorkspace } = migrateLegacyPaths();

  if (migratedConfig) {
    ok(`Migrated ${LEGACY_CONFIG_PATH} -> ${CONFIG_PATH}`);
  }
  if (migratedWorkspace) {
    ok(`Migrated ${LEGACY_WORKSPACE_PATH} -> ${WORKSPACE_PATH}`);
  }
  ...
  if (changed) {
    ...
    if (hasNewEntries) {
      ...
      info(`Removed retired file entries from ${CONFIG_PATH}.`);
    }
