...
CLI 会收集项目元数据，向所有支持的 AI TUI 安装 `update-agent-infra` 种子命令，并生成 `.agent-infra/config.json`。
...
```text
my-project/
├── .agent-infra/          # agent-infra 自身状态目录
│   ├── config.json        # 中央配置文件
...
生成出的 `.agent-infra/config.json` 是引导 CLI、模板系统和后续升级之间的中心契约。

### `.agent-infra/config.json` 示例
...
1. 在 `.agent-infra/config.json` 中把该路径从 `managed` 数组移除。
...
agent-infra 通过 Git tag 和 GitHub release 使用语义化版本号。当前安装的模板版本记录在 `.agent-infra/config.json` 的 `templateVersion` 字段中，方便人和 AI 工具在升级时都能基于同一个版本基线工作。
